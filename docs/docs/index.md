...
If you use AWS SSO credentials for Amazon Bedrock, mount your local `~/.aws`
directory and ensure `AWS_PROFILE`, `AWS_REGION`, and `AWS_SDK_LOAD_CONFIG=1`
are set in `.env.local` before running the container.
